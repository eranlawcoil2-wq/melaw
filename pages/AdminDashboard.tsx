  ...
  return (
    <div className="min-h-screen bg-slate-950 flex font-sans text-slate-200 overflow-hidden relative">
      ...
      <main className="flex-1 md:mr-64 p-4 md:p-8 overflow-y-auto min-h-screen">
                                         ...
                                         {editingForm.fields.map((field,i)=>(
                                             <div key={i} className="flex flex-col gap-2 bg-slate-800 p-3 rounded border border-slate-700">
                                                 ...
                                                 <div className="grid grid-cols-2 gap-4 bg-slate-950 p-2 rounded">
                                                     ...
                                                     {/* Client Email Toggle - Only for email types */}
                                                     {field.type === 'email' && (
                                                         <div className="flex items-center gap-2 p-1 bg-slate-900 rounded border border-slate-800">
                                                             <input 
                                                                type="checkbox" 
                                                                id={`isEmail-${i}`} 
                                                                checked={field.isClientEmail || false} 
                                                                onChange={e => {
                                                                    // Uncheck others first (simple approach)
                                                                    const newFields = editingForm.fields.map(f => ({...f, isClientEmail: false}));
                                                                    newFields[i].isClientEmail = e.target.checked;
        ...
        {/* ... Other Tabs ... */}
        {/* TIMELINE EDIT MODAL (REUSED FOR NEWS) */}
        {editingTimelineItem && (
            <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
