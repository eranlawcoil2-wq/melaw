...
const GOOGLE_SCRIPT_CODE = `function doPost(e) {
  ...
  try {
    ...
    if (action === 'submitForm') {
      ...
      for (var key in data) {
        if (key !== 'action' && key !== 'targetSheet' && key !== 'templateSheet' && key !== 'formName' && key !== 'submittedAt' && key !== 'submissionId' && key !== 'submitEmail') {
      ...
      if (data.submitEmail) {
          try {
              ...
              console.log("Email failed: " + emailErr.toString());
          }
      }
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex font-sans text-slate-200 overflow-hidden relative">
      ...
      <main className="flex-1 md:mr-64 p-4 md:p-8 overflow-y-auto min-h-screen">
        ...
        {/* ... (Previous Tab Contents - Team, Sliders, Payments, Config, etc. - should be preserved) ... */}
        {/* Render other tabs just as they were in the previous iteration to ensure full functionality */}
        {activeTab === 'sliders' && (
            <div className="space-y-6 animate-fade-in">
                     ...
                     <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
                         <div className="bg-slate-900 p-6 rounded border border-slate-700 w-full max-w-3xl h-[90vh] flex flex-col">
                             ...
                             <div className="flex-1 overflow-y-auto space-y-4 px-2">
                                 ...
                                 {/* NEW: Submit Email Field */}
                                 <div>
                                     <label className="block text-xs font-bold text-slate-400 mb-1">מייל לקבלת הטופס (אופציונלי)</label>
                                     <input 
                                        ...
                                        placeholder="office@melaw.co.il (ברירת מחדל: מייל המשרד)"
                                     />
                                     <p className="text-[10px] text-slate-500 mt-1">המערכת תשלח PDF לכתובת זו.</p>
                                 </div>
        ...
        {/* ... (Team, Payments, Config, Modals - Keeping existing implementations) ... */}
        {activeTab === 'team' && (
            <div className="space-y-6 animate-fade-in">
        ...
        {/* ... (Config and other modlas remain unchanged) ... */}
        {activeTab === 'config' && (
             <div className="space-y-6 animate-fade-in">
                <div className="bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-800 max-w-4xl">
