             ...
             <div className="bg-slate-900 rounded-xl border border-slate-700 w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                 ...
                 <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                     <div className="w-full md:w-1/3 bg-slate-900 border-l border-slate-800 p-4 overflow-y-auto space-y-6">
                         <div className="space-y-4"><h4 className="text-xs font-bold text-slate-500 uppercase tracking-wide">הגדרות כלליות</h4><div><label className="block text-xs font-bold text-slate-300 mb-1">שם הטופס (כותרת)</label><input className="w-full p-2 bg-slate-950 text-white rounded border border-slate-700 focus:border-[#2EB0D9] outline-none" value={editingForm.title} onChange={e=>setEditingForm({...editingForm, title: e.target.value})} placeholder="למשל: שאלון קליטת לקוח"/></div><div className="flex gap-2"><div className="flex-1"><label className="block text-xs font-bold text-slate-300 mb-1">סדר הופעה</label><input type="number" className="w-full p-2 bg-slate-950 text-white rounded border border-slate-700" value={editingForm.order || 99} onChange={e=>setEditingForm({...editingForm, order: Number(e.target.value)})}/></div></div><div><label className="block text-xs font-bold text-slate-300 mb-1">אימייל לקבלת הטופס</label><input className="w-full p-2 bg-slate-950 text-white rounded border border-slate-700 text-xs" value={editingForm.submitEmail || ''} onChange={e=>setEditingForm({...editingForm, submitEmail: e.target.value})} placeholder="office@melaw.co.il"/></div><div className="flex items-center gap-2 p-2 bg-slate-950 rounded border border-slate-800"><input type="checkbox" id="sendClientEmail" className="accent-[#2EB0D9] w-4 h-4" checked={editingForm.sendClientEmail || false} onChange={e=>setEditingForm({...editingForm, sendClientEmail: e.target.checked})}/><label htmlFor="sendClientEmail" className="text-xs text-slate-300 cursor-pointer select-none">שלח העתק ללקוח באופן אוטומטי</label></div></div>
